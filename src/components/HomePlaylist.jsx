    ...
    <div className='flex flex-col gap-4'>
		...
		<div className="flex flex-row flex-wrap gap-4 overflow-auto w-full">
			{dataList?.map((item, index) =>(
				<div className="flex flex-col gap-2 text-2xl font-semibold text-white w-[12rem] rounded-xl p-2 bg-zinc-500 hover:bg-zinc-700 cursor-pointer select-none group" key={index} onClick={() => {handlePlay(item)}}>
					<div className="flex relative">
						<img src={item.images[0].url} alt="" className="flex h-[12rem] w-[12rem] rounded-lg" />
						<div className="hidden justify-center items-center bg-green-600 rounded-full h-14 w-14 group-hover:flex hover:scale-110 absolute right-2 bottom-2">
